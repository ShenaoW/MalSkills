...
- `metadata.skillguard_subtype` must equal the provided subtype.
- `metadata.skillguard_evidence_type` must equal the provided evidence type.
- Do not use `skillguard_confidence`.
...
class EvidenceFeedbackAnalyzer:
    def __init__(self, cache_dir: str | Path | None = None, max_reviews: int = 8) -> None:
        default_cache = Path(".cache") / "skillguard_llm_feedback"
        configured = cache_dir or os.environ.get("SKILLGUARD_LLM_FEEDBACK_CACHE") or default_cache
        self.cache_dir = Path(configured)
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        self.max_reviews = max(1, int(os.environ.get("SKILLGUARD_LLM_FEEDBACK_MAX_REVIEWS", max_reviews)))
    ...
    def _validate_rule(
        ...
        if "skillguard_confidence" in lowered:
            return False, "Rule YAML must not emit skillguard_confidence."
        if f"skillguard_subtype: {candidate.subtype}" not in rule_yaml:
            return False, "Rule YAML metadata.skillguard_subtype is inconsistent."
        if f"skillguard_evidence_type: {candidate.evidence_type}" not in rule_yaml:
            return False, "Rule YAML metadata.skillguard_evidence_type is inconsistent."
...
- Use repository-style metadata keys:
  - `skillguard_subtype: {candidate.subtype}`
  - `skillguard_evidence_type: {candidate.evidence_type}`
- Do not include `skillguard_confidence`
...
"""
    ...
    def _suggested_rule_path(self, artifact_type: str, evidence_type: str) -> str:
        ...
        if artifact_type == "prompt":
            bucket = "markdown"
        return f"skillguard/rules/semgrep/{bucket}/{evidence_type}.yml"
