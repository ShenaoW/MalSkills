...
class YasaAdapter:
    ...
    CHECKER_CONFIG = {
        "javascript": ("object_binding_js_input", None),
        "python": ("object_binding_python_input", "SKILLGUARD_YASA_UAST_SDK"),
        "go": ("object_binding_go_input", None),
        "java": ("object_binding_java_input", None),
    }
    ...
    def _fenced_code_targets(self, artifact: ArtifactRecord) -> list[AnalysisTarget]:
        ...
        for index, (language_name, body, start_line, _) in enumerate(iter_code_fences(artifact.content or "")):
            ...
            source_path = (parent / ".skillguard_yasa" / source_name).as_posix()
            targets.append(
                AnalysisTarget(
                    artifact_id=artifact.artifact_id,
                    artifact_path=artifact.relative_path,
