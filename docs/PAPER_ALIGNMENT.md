...
| Declarative reasoner | Relational facts and declarative graph patterns | Built-in rules are JSON graph queries interpreted by a bounded matcher with subtype roles, attribute constraints, directed flow, and object identity. Souffle remains an export format rather than the execution engine | Aligned in behavior; different engine |
